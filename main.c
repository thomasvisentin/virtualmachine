        ...
        while(guardia){
            switch(memoria[IP++]){
                ...
                case 1: 								//display
                    ...
                    IP++; /*IP++ come era prima dentro il printf da problemi*/
                    break;
			    ...
			    if(stack[SP-1] < 0 && (registri[memoria[IP]] > 0 && registri[memoria[IP+1]] > 0))  //verifica overflow
					{
						printf("Overflow\n");
						guardia = 0;
					}
                ...
                case 31: 										//sub
                    stack[SP++] = registri[memoria[IP]] - registri[memoria[IP+1]]; 
					if ((stack[SP-1] < registri[memoria[IP]]) != (registri[memoria[IP+1]] > 0)){       //verifica overflow
                        printf("overflow\n");
						guardia = 0;
					}
					IP=IP+2;
                ...
                case 32: 										//mult
                    stack[SP++] = registri[memoria[IP]] * registri[memoria[IP+1]]; 
					if (stack[SP-1] != 0 && stack[SP-1] /  registri[memoria[IP]] != registri[memoria[IP+1]]) {     /*verifica overflow/underflow*/
                        printf("overflow\n");
                        guardia = 0;
                    }
					IP=IP+2;
